    ...
    hiddenInput.addEventListener("change", async () => {
        ...
        uploadStatus.innerHTML = `<span class="text-blue-600">Uploading "${file.name}"...</span>`;

        const formData = new FormData();
        formData.append("file", file);

        try {
            const uploadUrl = `${API_BASE}/upload/${currentThreadId}`;
            ...
            addUploadedFileToUI(file.name, data.file_id, data.active_file_id);

            uploadStatus.innerHTML = `
                <span class="text-green-600">
                    Uploaded: ${file.name} (Vectors: ${data.total_vectors})
                </span>
            `;
    ...
    function addUploadedFileToUI(filename, fileId, activeFileId) {

        const fileItem = document.createElement("div");
        fileItem.className =
            "p-2 bg-gray-200 rounded-md text-sm flex justify-between items-center";
        ...
        fileItem.innerHTML = `
            ...
            ${
                isActive
                    ...
                    : `<span class="text-gray-500 text-xs">Uploaded</span>`
            }
        `;
    ...
    function renderToolEvent(tool, args = {}) {
        ...
        // Only show executing message for these tools
        const allowedTools = ["add_expense", "list_expenses"];

        // If tool is NOT in allowed list → do NOT render anything
        if (!allowedTools.includes(tool)) {
            console.log(`Skipping tool execution bubble for: ${tool}`);
            return;
        }

        // Otherwise render the tool execution bubble
        const wrapper = document.createElement("div");
        ...
        const bubble = document.createElement("div");
        bubble.className =
            "bg-yellow-100 text-gray-900 p-3 rounded-lg max-w-lg font-mono text-sm";

        let argsDisplay = "";
        if (args && Object.keys(args).length > 0) {
            argsDisplay = `<div class="text-xs mt-1 text-gray-600">
    ...
    async function sendToBackend(text) {
        return new Promise(async (resolve) => {
            try {
                ...
                if (buffer.trim() !== "") {
                    try {
                        const payload = JSON.parse(buffer.replace("data:", "").trim());
